...
# 数字の色を変更する関数
def change_needle_color(new_color):
    global color_index
...
def draw_clock():
    ...
    w.create_line(WINwidth / 2, WINheight / 2, WINwidth / 2 + H_y, WINheight / 2 - H_x, width=H_LINEwidth, fill=rainbow_colors[color_index])  # 時針
    w.create_line(WINwidth / 2, WINheight / 2, WINwidth / 2 + M_y, WINheight / 2 - M_x, width=M_LINEwidth, fill=rainbow_colors[color_index])  # 分針
...
for i, color in enumerate(rainbow_colors):
    color_button = Button(Clock, text=color, command=lambda color=color: change_needle_color(color))
